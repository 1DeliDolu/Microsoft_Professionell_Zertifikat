...
Bu laboratuvarda birkaç şey yapacağız. Önce, *EPCOR* ile çalışmak için geliştirme ortamımızı kuracağız ve veritabanı tablolarını temsil edecek entity sınıflarını oluşturup yapılandıracağız.

Veritabanı ilişkilerini bağlam ( *context* ) içinde yönetmek için *dbContext* kullanacağız. Ardından, veritabanında şemayı üretmek için migration’ları uygulayacağız ve sonra veritabanı modelini veri ekleyerek ve sorgulayarak test edeceğiz.
...
Klasöre geçeceğiz ve ardından, yapmamız gereken tüm işleri yapabilmemiz için uygulamaya **SQLite** ve **Entity Framework Core tools** ekleyeceksiniz.
...
Burada, ayarlayacağınız her property için türleri eklemek isteyeceksiniz. Örneğin, **EmployeeID** `Int`.
...
Artık migration’larımız var. Bir **migrations** klasörü oluşturulduğunu görebilirsiniz ve migration’lar hazır.
...
Entity Framework ve bu migration’ları kullanarak, neyin oluşturulacağını anlamak için class’ları kullandık. Migration’ları oluşturdu ve ardından migration’ları kullanarak veritabanını güncelledi ve oluşturdu.

Yine, bu modelimizde tanımlanmıştı. Entity Framework’ün neyi oluşturacağını bilmesinin yolu bu.
...
**program.cs** içinde birkaç şey oluşturdum. Yeni bir department oluşturacağız. Departmanın adı **Engineering** olacak ve bu departmana birkaç employee ekleyeceğiz.
...
Bu iki employee’yi veritabanımıza ekledik. Veritabanını sorguladık ve count değerini geri aldık.
